...
function Get-CleanRenderCalendar {
...
function Get-CleanVocabSchedule {
...
$calCode = (Get-Content $MyInvocation.MyCommand.Path) | Where-Object { $_ -match "^\s{4}" } | ForEach-Object { $_ -replace "^\s{4}", "" }
# This is getting too complex. I'll just hardcode the strings in the script.
...
$newLines = $lines[0..2625] + $calCode + $lines[2840..7695] + $vocabCode + $lines[7724..$($lines.Length-1)]
# Fix version at start
